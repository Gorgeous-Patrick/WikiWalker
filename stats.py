...
def estimate_transfer_time(
    paths: list[list[str]], sched: list[list[str]], moved: list[str]
) -> float:
...
plt.plot(
    [
        estimate_transfer_time(
            estimation.paths[idx * step : (idx + 1) * step],
            iterative[idx].sched,
            iterative[idx].moved_pages,
        )
        for idx in range(len(iterative))
    ],
    linestyle="-",
...
plt.plot(
    [
        estimate_transfer_time(
            estimation.paths[idx * step : (idx + 1) * step], estimation.rand_sched, []
        )
        for idx in range(len(iterative))
    ],
    linestyle="-",
...
# Add a title
plt.title(
    f"{len(list(cache_path.iterdir()))} Pages, {len(list(run_path.iterdir()))} Walker runs"
)
